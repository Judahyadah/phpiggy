...
class TransactionService
{
    ...
    public function create(array $formData)
    {
        ...
        $this->db->query(
            "INSERT INTO transactions(user_id, description, amount, date) VALUE(:user_id, :description, :amount, :date)",
            [
    ...
    public function getUserTransactions(int $length, int $offset )
    {
        ...
        $transactions = $this->db->query(
            "SELECT *, DATE_FORMAT(DATE, '%Y-%m-%d') as formatted_date FROM transactions WHERE user_id = :user_id AND description LIKE :description LIMIT {$length} OFFSET {$offset}", $params
        )->findAll();
        ...
        $transactionCount = $this->db->query(
            "SELECT COUNT(*) FROM transactions WHERE user_id = :user_id AND description LIKE :description", $params
        )->count();
